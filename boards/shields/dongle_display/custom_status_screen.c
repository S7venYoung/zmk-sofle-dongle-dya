...
static struct zmk_widget_bmw_theme bmw_theme_widget;
...
static void apply_runtime_display_settings(struct k_work *work) {
    ...
    bool bmw_theme = theme == 2;

    set_widget_visible(zmk_widget_bmw_theme_obj(&bmw_theme_widget), bmw_theme);

    zmk_widget_output_status_set_compact(&output_status_widget, yads_theme || bmw_theme);
    if (yads_theme) {
    ...
    set_widget_visible(zmk_widget_wpm_status_obj(&wpm_status_widget),
                       yads_theme || bmw_theme || zmk_display_settings_wpm_enabled());
    if (yads_theme || bmw_theme) {
    ...
    set_widget_visible(zmk_widget_key_stats_status_obj(&key_stats_status_widget),
                       !yads_theme && !bmw_theme && zmk_display_settings_key_stats_enabled());
...
#if IS_ENABLED(CONFIG_ZMK_DONGLE_DISPLAY_BONGO_CAT)
    bool bongo_cat_enabled = !yads_theme && !bmw_theme && zmk_display_settings_bongo_cat_enabled();
    ...
    if (yads_theme) {
        lv_obj_align(zmk_widget_modifiers_obj(&modifiers_widget), LV_ALIGN_CENTER, 0, 8);
    } else {
    ...
    set_widget_visible(zmk_widget_hid_indicators_obj(&hid_indicators_widget),
                       !yads_theme && zmk_display_settings_modifiers_enabled());
#endif
#endif
...
lv_obj_t *zmk_display_status_screen() {
    ...
    zmk_widget_bmw_theme_init(&bmw_theme_widget, screen);
    lv_obj_align(zmk_widget_bmw_theme_obj(&bmw_theme_widget), LV_ALIGN_CENTER, 0, 0);
    set_widget_visible(zmk_widget_bmw_theme_obj(&bmw_theme_widget), false);
