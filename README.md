...
| `bmw-theme` | 仅保留低占用 BMW 大括号仪表的精简固件 | 开发测试中 |
| `fighting-theme` | 左右键盘输入速度驱动的像素对战单主题固件 | 开发测试中 |
...
| 设置 | 作用 | 范围 |
| --- | --- | --- |
| `display_theme` | OLED 主题 | `2`=圆形仪表，`3`=BMW 折线仪表（旧值 `0/1` 自动按 `2` 处理） |
...
`bmw-theme` 只保留 BMW 大括号仪表，不包含 Classic、YADS 或圆形仪表，也不再提供 `display_theme` 运行时切换。

静态的大括号与刻度合并为一张约 1032 字节的 1-bit 图片（128×64）。大括号中间折角改为短竖线，保留左右动态指针，并在表盘内直接显示当前 WPM 与峰值。圆形 Arc、速度表图片、动画和 DYA 显示设置子系统均已移除，以降低 Flash 与 LVGL 堆占用。

布局：

- 左侧大括号：当前 WPM 数字与动态指针
- 右侧大括号：最近峰值 WPM 数字与动态指针，峰值保持约 3 秒
- 顶部中央：仅显示当前连接方式，USB 为 `USB`，蓝牙为 `BT`
- 中央：当前层级缩写（`D`=BASE、`N`=NAV、`R`=SYS）
- 底部中央：累计按键总数，仅显示数字，不带 `T` 前缀
- 最底部左右：左右手电量横条，不显示电量数值

### Fighting 单主题固件

`fighting-theme` 从 `bmw-theme` 独立建立，只编译像素对战屏幕，不包含 BMW
大括号、圆形仪表、Bongo Cat、层级、WPM 或按键总数等显示主题。

- 左半键盘按键驱动左侧角色，右半键盘按键驱动右侧角色。
- 最近输入越密集，角色从普通攻击依次升级为重击和连击。
- 动画固定以 10 FPS 刷新，按键事件只更新轻量状态，不直接重绘屏幕。
- 顶部左右横条分别显示左右键盘电量，外观沿用格斗游戏血条。
- 顶部中央 `U` 表示 USB，`B` 表示蓝牙。
- 左右角色共用一套压缩关键帧，左侧运行时镜像，避免保存两份素材。
