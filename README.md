...
| 设置 | 作用 | 范围 |
| --- | --- | --- |
| `display_theme` | OLED 主题 | `0`=Classic，`1`=YADS，`2`=圆形仪表，`3`=BMW 折线仪表 |
...
`4.1-display-themes` 在保留当前 Classic 猫咪主题的基础上增加 YADS 信息主题、圆形汽车仪表主题和 BMW 折线仪表主题。通过 DYA Studio 将 `display_theme` 写为 `0`、`1`、`2` 或 `3` 后立即切换，点击页面顶部的 `Save` 后持久保存。

YADS 主题底部的左右副手电量始终显示：左右各使用相同字号显示数字，下方横线长度表示剩余电量；尚未收到电量或断开连接时显示 `X`。

电量横条保持固定长度和 1 像素空心边框；随着电量降低，空心区域从屏幕中心侧向左右外侧扩展，靠屏幕外侧的端帽保持实心。左右电量模块分别贴近 OLED 的左右边缘和底边显示。

YADS 主题布局：

- 左上：实时 WPM
- 右上：USB/BLE 连接状态
- 中间：居中显示最近输入的 5 个英文字母
- 连续 5 秒没有按键输入时：清除字母历史并显示当前层级
- 按住切层键时：中间显示当前层级和该层输入的字母
- 字母下方：仅在修饰键按下期间居中显示；同时按下多个修饰键时从中心向两侧展开，并为每个已按住的修饰键保留下划线
- 左下与右下：左右手键盘电量

YADS 主题仅显示最近 5 个 HID 字母 `A`–`Z`，不会解析中文输入法最终输出的汉字。它同样可能显示密码中键入的字母；在公共环境使用时可切回 Classic 主题。Classic 主题的按键统计、猫动画和原布局保持不变。

仪表盘主题（`display_theme=2`）采用汽车仪表风格：左右两侧竖向电量条，中间只显示当前层级缩写（`D`=BASE、`N`=NAV、`R`=SYS），左半圆显示当前 WPM，右半圆显示本次开机以来的峰值 WPM，底部显示 USB/BLE 状态和 `T` 总按键统计。该主题不显示输入字符、修饰键、猫动画或每日统计。

BMW 折线仪表主题（`display_theme=3`）采用两只相向的折线括号仪表：左侧括号内的指针表示当前 WPM，右侧括号内的指针表示最近峰值 WPM；峰值保持约 3 秒后会随当前输入速度回落。中间显示当前层级缩写（`D`=BASE、`N`=NAV、`R`=SYS），底部以左右两条横线显示左右手电量，并以小号文字显示 USB/BLE 状态和 `T` 总按键统计。该主题不显示 WPM 数字。
