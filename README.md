...
`theme` 分支只保留两套低占用仪表主题：圆形汽车仪表（`display_theme=2`）和 BMW 折线仪表（`display_theme=3`）。Classic/YADS 主题及其动画、修饰符和输入字符模块已从接收器固件中移除，避免 1 MB Flash 设备因固件过大而无法启动。旧固件曾保存的 `display_theme=0/1` 会自动回退到圆形仪表。
...
BMW 大括号仪表主题（`display_theme=3`）按照草图使用左右镜像的五段折线大括号，每侧带 8 条刻度：左侧指针表示当前 WPM，右侧指针表示最近峰值 WPM；峰值保持约 3 秒后会随当前输入速度回落。中央增加一组对称的宝马双肾大括号轮廓，并在其中显示当前层级缩写（`D`=BASE、`N`=NAV、`R`=SYS）。底部以左右两条横线显示左右手电量，并以小号文字显示 USB/BLE 状态和 `T` 总按键统计。该主题不显示 WPM 数字。
