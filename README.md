...
静态的大括号、刻度和中央双肾合并为一张约 776 字节的 1-bit 图片。运行时只保留左右两根 WPM 指针、中央层级、USB/BLE 文本、累计按键和左右手电量条。圆形 Arc、速度表图片、图标动画、第二套 WPM widget、其他电量布局和 DYA 显示设置子系统均已移除，以降低 Flash 与 LVGL 堆占用。

布局：

- 左侧大括号指针：当前 WPM
- 右侧大括号指针：最近峰值 WPM，保持约 3 秒
- 中央双肾：当前层级缩写（`D`=BASE、`N`=NAV、`R`=SYS）
- 底部左侧：USB/BLE 输出状态
- 底部右侧：`T` 累计按键
- 最底部左右：左右手电量横条
